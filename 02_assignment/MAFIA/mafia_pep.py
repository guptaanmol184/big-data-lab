""" MAFIA: A Maximal Frequent Itemset Algorithm (With PEP - Parent Equivalence Pruning)
...
def _mafiaAlgorithm(current_node, MFIs, transactions, min_support_count):
	...
	for i, item in enumerate(current_node.tail):
		...
		new_node_support_cnt = transactions.countSupport(new_node_head)

		if new_node_support_cnt == current_node.support_count:
			current_node.head += (item,)

		elif  new_node_support_cnt >= min_support_count:
			is_leaf = False
...
if __name__ == '__main__':
	...
	""" Custom Example for MAFIA with PEP """
	transactions = [
		{'A', 'C', 'D'},
		{'A', 'C', 'E'},
		{'A', 'B', 'C', 'E'},
